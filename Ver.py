...
if PARAMETROS is not None:
    ...
    if opcion == "Correr modelo":
        ...
        st.markdown("""
        <style>
        .stTabs [data-baseweb="tab-list"] { justify-content: space-evenly; }
        .stTabs [data-baseweb="tab"] { flex-grow: 1; text-align: center; }
        .stTabs [data-baseweb="tab"] > div{ font-size: 21px; font-family: Verdana; font-weight: bold; }
        </style>
        """, unsafe_allow_html=True)
        ...
        with tab1:
            ...
            def cargar_parametros(PARAMETROS):
                ...
                AD_list = []
                for i_ in range(1, 6):
                    df_ = pd.read_excel(PARAMETROS, sheet_name=f"AD_tis{i_}", header=0, index_col=0)
                    AD_list.append(df_.values)
                AD_tis = np.stack(AD_list, axis=0)
                AD_tis = np.transpose(AD_tis, (1, 2, 0))

                C_list = []
                for i_ in range(1, 6):
                    df_ = pd.read_excel(PARAMETROS, sheet_name=f"C_ics{i_}", header=0, index_col=0)
                    C_list.append(df_.values)
                ...
                # OJO: aquí tu tamaño está fijo (38,5,30,2,51); lo dejo tal cual
                ...
                for t in range(38):
                    for s in range(5):
                        for d in range(30):
                            for j in range(2):
                                for i in range(51):
                                    # CORREGIDO: índices 0-based, sin -1
                                    denom = TA_tis[t, i, s]
                                    if denom == 0:
                                        A2tsdji[t, s, d, j, i] = 0
                                    else:
                                        A2tsdji[t, s, d, j, i] = np.floor(TJ_tdj[t, d, j] / denom)
                ...
                TD_tdj = np.stack([TD_tdj1.values, TD_tdj2.values], axis=0)
    ...
    # =========================
        ...
        # =========================
        # TAB 1: VISUALIZACIÓN (igual a tu lógica)
        # =========================
        with tab1:
            st.write("Asignación")
            archivo = "Xtsdji.csv"
            ...
            if os.path.exists(archivo) and os.path.exists(archivo2):
                ...
                JORNADAS = pd.DataFrame([["J1", "AM"], ["J2", "PM"]], columns=["Codigo", "Nombre"])
                ...
                Xtsdji = Xtsdji.merge(TRABAJADORES, left_on="Trabajador", right_on="Codigo", how="left")
                ...
                Xtsdji = Xtsdji.merge(SEDES, left_on="Sedes", right_on="Codigo", how="left")
                ...
                Xtsdji = Xtsdji.merge(JORNADAS, left_on="Jornada", right_on="Codigo", how="left")
                ...
                Ptcsdji = Ptcsdji.merge(TRABAJADORES, left_on="Trabajador", right_on="Codigo", how="left")
                ...
                Ptcsdji = Ptcsdji.merge(SEDES, left_on="Sedes", right_on="Codigo", how="left")
                ...
                Ptcsdji = Ptcsdji.merge(JORNADAS, left_on="Jornada", right_on="Codigo", how="left")
                ...
                nuevo_orden = ["Trabajador", "Consultorio", "Sede", "Fecha", "Jornada", "Especialidad", "Día"]
                Ptcsdji = Ptcsdji[nuevo_orden]

                rango = st.date_input("Selecciona un rango de fechas", [Xtsdji["Fecha"].min(), Xtsdji["Fecha"].max()])
                ...
                if len(rango) == 2:
                    ...
                    if sede_sel == "UIS":
                        ...
                        resultado = pd.concat([filtrado_P[filtrado_P["Sede"] != "UIS"], filtrado_X[filtrado_X["Sede"] == "UIS"]])
                    else:
        ...
        # =========================
        # TAB 2: ESTADÍSTICAS (TABLA POR CANTIDAD)
        # =========================
        with tab2:
            ...
            # =====================================================
            # AUXILIARES (tu bloque original, sin tocar)
            # =====================================================
            ...
            # servicios 14 y 15 no duplican por t,s,fecha,jornada
            ...
            aux_diario = aux_diario[
                ["Sede", "Fecha", "Jornada",
                 "Total_aux_sede", "Total_aux_3sedes", "Capacidad_pool",
                 "% ocupación_pool", "% desocupación_pool"] + aux_cols
            ]
            ...
            styled_aux = (
                aux_diario.round(2).style.applymap(color_ocupacion, subset=["% ocupación_pool"])
            )
